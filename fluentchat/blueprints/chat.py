...
from fluentchat.extensions import socketio, db
from fluentchat.forms import ProfileForm
from fluentchat.models import Message, User
from fluentchat.utils import to_html, flash_errors
...
@chat_bp.route('/')
def home():
    amount = current_app.config['FLUENTCHAT_MESSAGE_PER_PAGE']
...
def get_messages():
    ...
    pagination = Message.query.order_by(Message.timestamp.desc()).paginate(
        page, per_page=current_app.config['FLUENTCHAT_MESSAGE_PER_PAGE'])
